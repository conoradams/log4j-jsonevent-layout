...
public class JSONEventLayoutV1Test {
	...
	static final String renameSingleField = new String("field1:renamed_field");
	static final String renameMDCField = new String("mdc-foo:renamed_mdc_field");
	static final String renameNestedMDCField = new String("mdc-foo-bar:renamed_mdc_nested_field");
	...
	@Test
	public void testJSONEventLayoutHasRenamedField1ToRenamedField() {
		JSONEventLayoutV1 layout = (JSONEventLayoutV1) appender.getLayout();
		
		String prevUserData = layout.getUserFields();
		layout.setUserFields(userFieldsSingle);
		
		String prevRenameFields = layout.getRenameFields();
		layout.setRenameFields(renameSingleField);
		...
		Object obj = JSONValue.parse(message);
		JSONObject jsonObject = (JSONObject) obj;
		Assert.assertTrue("Event does not contain field 'renamed_field'" , jsonObject.containsKey("renamed_field"));
		Assert.assertEquals("Event does not contain value 'value1'", "value1", jsonObject.get("renamed_field"));

		layout.setUserFields(prevUserData);
		layout.setRenameFields(prevRenameFields);
	}
	...
	@Test
	public void testJSONEventLayoutHasRenamedMDC() {
		JSONEventLayoutV1 layout = (JSONEventLayoutV1) appender.getLayout();
		String prevRenameFields = layout.getRenameFields();
		layout.setRenameFields(renameMDCField);
		layout.setKeepOriginalMaps(true);
		...
		Assert.assertTrue("Event does not contain field 'renamed_field'" , jsonObject.containsKey("renamed_mdc_field"));
		Assert.assertEquals("Event does not contain value 'bar'", "bar", jsonObject.get("renamed_mdc_field"));
	...
	@Test
	public void testJSONEventLayoutHasNoMDC_RemoveOriginalMaps() {
		JSONEventLayoutV1 layout = (JSONEventLayoutV1) appender.getLayout();
		String prevRenameFields = layout.getRenameFields();
		layout.setRenameFields(renameMDCField);
		layout.setKeepOriginalMaps(false);
		...
		Assert.assertNull("MDC should not be here", mdc);
		
		Assert.assertTrue("Event does not contain field 'renamed_mdc_field'" , jsonObject.containsKey("renamed_mdc_field"));
		Assert.assertEquals("Event does not contain value 'bar'", "bar", jsonObject.get("renamed_mdc_field"));
	...
	@Test
	public void testJSONEventLayoutHasNoNestedMDC_RemoveOriginalMaps() {
		JSONEventLayoutV1 layout = (JSONEventLayoutV1) appender.getLayout();
		String prevRenameFields = layout.getRenameFields();
		layout.setRenameFields(renameNestedMDCField);
		layout.setKeepOriginalMaps(false);
		
		HashMap<String, String> nestedMdc = new HashMap<String, String>();
		nestedMdc.put("bar","baz");
		nestedMdc.put("cpa","123");
		MDC.put("foo",nestedMdc);
		logger.warn("I should have MDC data in my log");
		String message = MockAppenderV1.getMessages()[0];
		...
		Assert.assertTrue("Event does not contain field 'renamed_mdc_nested_field'" , jsonObject.containsKey("renamed_mdc_nested_field"));
		Assert.assertEquals("Event does not contain value 'baz'", "baz", jsonObject.get("renamed_mdc_nested_field"));
		Assert.assertTrue("Event does not contain field 'mdc-foo-cpa'" , jsonObject.containsKey("mdc-foo-cpa"));
		Assert.assertFalse("Event does contain field 'mdc-foo-bar' instead of renamed field" , jsonObject.containsKey("mdc-foo-bar"));
